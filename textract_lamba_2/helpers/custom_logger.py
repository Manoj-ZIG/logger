...
'collecting textract':'Text extraction',
'textract response is':'Text extraction',
'collecting table extraction' : 'Table extraction',
'table extraction response is' : 'Table extraction',
...
class S3Logger:
    ...
    def _prepare_content(self, timestamp, arl, file_name, category, message):
        ...
        values = f"{timestamp}|textract_lambda_2|{arl}|{file_name}|{category}|{message}"
        return f"{headers}\n{values}"
