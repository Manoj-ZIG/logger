...
#     from . import date_finder as dtf
#     from . import time_finder as tf

try:
    import date_time_module.date_finder as dtf
    import date_time_module.time_finder as tf
except ModuleNotFoundError as e:
    from . import date_finder as dtf
    from . import time_finder as tf
...
class DatetimeExtractor:
    ...
    @staticmethod
    def get_date_tag(datetime_str, processed_date_tags, date_start, prev_date_end, lookbehind_idx=18, ):
        """ 
        ...
        # make the lookbehind more dynamic instead of any int, it will changed based on nearest space as we don't want our string get ripped off!
        space_idx = [i.start() for i in re.finditer(r"\s",datetime_str_original)]
        thresh = date_start-lookbehind_idx
        adjust_lookbehind_index = max(space_idx, key=lambda x: x if (
            x-thresh) < 0 else 0) if space_idx else 0

        if date_start <= lookbehind_idx or thresh <= adjust_lookbehind_index:
            datetime_str = datetime_str[0:date_start]
        elif prev_date_end and prev_date_end > (date_start-lookbehind_idx):
            datetime_str = datetime_str[prev_date_end: date_start]
        else:
            datetime_str = datetime_str[adjust_lookbehind_index:date_start]
        ...
        detected_tags = [
            i for i in processed_date_tags if i in datetime_str]
        if detected_tags:
            return detected_tags[0]

    @staticmethod
    def process_date_tags(date_tags):
        processed_date_tags = [str(tag).lower().strip() for tag in date_tags]
        processed_date_tags = [re.sub(r"[^a-zA-Z]", " ", str(tag)).strip() for tag in processed_date_tags]
        processed_date_tags = sorted(processed_date_tags, key=len, reverse=True)
        return processed_date_tags
    ...
    def get_date_time_from_corpus(corpus, date_tags) -> list:
        ...
        for detected_date in reversed_date_ls:
            ...
            tag = DatetimeExtractor.get_date_tag(
                corpus, date_tags, date_start, prev_date_end)
            if len(time_ls) > 0:
    ...
    @staticmethod
    def get_date_time_from_corpus_v2(corpus, date_tags, restrict_dict={'day': (1, 31), 'month': (1, 12), 'year': (1900, 2026), 'year_': (1, 99)}):
        extracted_date = DatetimeExtractor.validate_date(corpus)

        # supress datetime (as it's detected False Positive as well so keep only True date/time)
        suppressed_date = DatetimeExtractor.suppress_datetime(extracted_date)
        processed_date_tags = DatetimeExtractor.process_date_tags(date_tags)
        final_date_list = []
        prev_date_end = None
        for detected_date in suppressed_date:
            date, _, date_start, date_end = detected_date
            tag = DatetimeExtractor.get_date_tag(corpus, processed_date_tags, date_start, prev_date_end)
            extracted_time = DatetimeExtractor.validate_time(
                corpus[date_end:date_end+50])
            suppressed_time = DatetimeExtractor.suppress_datetime(
                extracted_time)
            if len(suppressed_time) > 0:
                time, _, time_start, time_end = suppressed_time[0]
                if time:
                    final_date_list.append((date, time, tag))
            else:
                final_date_list.append((date, None, tag))
