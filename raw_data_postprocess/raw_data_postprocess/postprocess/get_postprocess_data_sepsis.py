...
class GetPostProcessData:
            ...
            for i, row in pre_df.iterrows():
                ...
                if attr == 'Sodium' and  score>=self.attribute_thresholds['Sodium']:
                    post_process_test_result, test_name_unit = Postprocess.sodium_postprocess(
                        test_result)
                    test_name_ = attr
                    # ref_range, abnormal_flag = PneumoniaAnalysis
                ...
                elif attr == 'Urine Volume (e.g. I&O\'s)':
                    post_process_test_result, test_name_unit = Postprocess.urine_volume_postprocess(
                        test_result)
                    test_name_ = attr
                    ...
                    sbp_match = process.extractOne(test_name.strip().lower(), ['systolic blood pressure','sbp'], scorer=fuzz.token_sort_ratio)
                    dbp_match = process.extractOne(test_name.strip().lower(), ['diastolic blood pressure','sbp'], scorer=fuzz.token_sort_ratio)
    ...
    def get_postprocess_data_for_excerpt(self, pre_df, p_obj, default_year, attribute_ls):
        ...
        numeric_df = pre_df[pre_df['TestName'].isin(
            [ 'Procalcitonin', 'Fever', 'Platelets', 'Mean Arterial Pressure', 'PT', 'INR', 'APTT', 'Urine Volume (e.g. I&O\'s)', 'Serum Creatinine', 'Urine Creatinine',
    ...
    def get_postprocess_df(self,s3_c,bucket_name, save_path, post_process_obj, default_year, raw_data_df, raw_data_excerpt_df, attribute_ls, path_to_save_logs):
        ...
        if raw_data_df.shape[0] > 0 or raw_data_excerpt_df.shape[0] > 0:
            ...
            if len(post_process_data)>0:
                ...
                logging.info('concatenate table df and numeric df from excerpts ...')
            else:
