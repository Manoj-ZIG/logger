...
class EncephalopathyAnalysis:
    def __init__(self, postprocess_data,post_process_excerpt_df, test_name_range_dict, vital_regex_list):
        ...
        self.encp_lab_list = test_name_range_dict.keys()
    ...
    def get_ref_range_and_boolean_flag(test_name, test_result, test_name_range_dict):
        if test_result: 
            ...
            if not (test_name_range_dict.get(test_name).get('encephalopathy')[1] == None or test_name_range_dict.get(test_name).get('encephalopathy')[0] == None):
                if test_result > test_name_range_dict.get(test_name).get('encephalopathy')[1] or test_result < test_name_range_dict.get(test_name).get('encephalopathy')[0]:
                    return test_name_range_dict.get(test_name).get('encephalopathy'), 1
                else:
                    return test_name_range_dict.get(test_name).get('encephalopathy'), 0
            else:
                if test_name_range_dict.get(test_name).get('encephalopathy')[1]:
                    if test_result > test_name_range_dict.get(test_name).get('encephalopathy')[1]:
                        return test_name_range_dict.get(test_name).get('encephalopathy'), 1
                    else:
                        return test_name_range_dict.get(test_name).get('encephalopathy'), 0
                elif test_name_range_dict.get(test_name).get('encephalopathy')[0]:
                    if test_result < test_name_range_dict.get(test_name).get('encephalopathy')[0]:
                        return test_name_range_dict.get(test_name).get('encephalopathy'), 1
                    else:
                        return test_name_range_dict.get(test_name).get('encephalopathy'), 0
        else:
        ...
        if self.postprocess_data.shape[0] > 0:
            for test_name, test_range_dict in self.test_name_range_dict.items():
                ...
                if test_result_list_ and test_name not in return_dict.keys():
                    # update encephalopathy flag list
                    if test_name in  self.encp_lab_list:
        ...
        if self.post_process_excerpt_df.shape[0]>0:
            ...
            for test_excp, flag in grp.to_dict().items():
                if test_excp not in return_dict and flag==1:
                    return_dict[test_excp] = {'encephalopathy': 1}
