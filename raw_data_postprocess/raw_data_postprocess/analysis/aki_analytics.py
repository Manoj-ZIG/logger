...
class AKIAnalysis:
    ...
    def get_creatine_boolean(self):
        ...
        # test_result_df = self.postprocess_data[self.postprocess_data['TestName']
        #                                        == 'Serum Creatinine']
        # if test_result_df.shape[0] > 0:
        #     creat_flag = test_result_df['TestResult'].apply(
        #         lambda x: 1 if x >= min(test_result_df['TestResult'])*1.5 else 0).max()

        #     creat_flag_ = 0
        #     for idx, test_1 in reversed(list(enumerate(test_result_df['TestResult'].to_list()))):
        #         for sub_idx, test_2 in enumerate(test_result_df['TestResult'].to_list()):
        #             if test_1-test_2 >= 0.3 and (pd.to_datetime(test_result_df['TestDateTime'].iloc[idx]) >
        #                                          pd.to_datetime(test_result_df['TestDateTime'].iloc[sub_idx])):
        #                 creat_flag_ = 1

        #     creat_flag__ = int(any((creat_flag_, creat_flag)))
        #     return {'Serum Creatinine': {
        #         'aki': int(creat_flag__),
        #         'atn': 0}}
        # else:
        #     pass
                ...
                if test_result_list_ and test_name not in return_dict.keys():
                    ...
                    if test_name in ['Urine Osmolality', 'Urine Specific Gravity', 'Blood Urea Nitrogen (BUN)', 'Urine Creatinine',]:
                        ...
                        if max(test_result_list_) > test_range_dict.get('atn'):
                            ...
                            return_dict[test_name] = {'aki': 0, 'atn': 0}
                    elif test_name in ['Bilirubin', 'Potassium', 'Sodium', "Urine Protein"]:
