...
class EncephalopathyLabsPanel:
    ...
    def get_page_meta_data(self):
        ...
        try:   
            ...
            print("S3 client initialized successfully using IAM role in encephalopathy_table_page_detection.py.")
        except Exception as e:
            print(f"Failed to initialize S3 client with IAM role: {str(e)} in encephalopathy_table_page_detection.py.")
            if aws_access_key_id and aws_secret_access_key:
                self.s3_c = boto3.client('s3', 
                                    aws_access_key_id=aws_access_key_id,
                                    aws_secret_access_key=aws_secret_access_key)
                print("S3 client initialized successfully using manual keys in encephalopathy_table_page_detection.py.")
            else:
    ...
    def check_section(self, sec_list, sub_sec_ls):
        ...
        for section in sec_list:
            if str(section) in self.master_section_list:
    ...
    def table_checker(self, df):
        ...
        for idx, row in df.iterrows():
            if len(row['corpus_date']) >= 1 and len(row['table_element_match_list']) >= 1 and self.check_section(row['section'], row['zai_mapped_sub_section']):
                tbl_ls.append(1)
            else:
                tbl_ls.append(0)
        df['is_table'] = tbl_ls
