...
class TropCkmb:
    ...
    def get_page_meta_data(self):
        ...
        try:   
            ...
            print("S3 client initialized successfully using IAM role in torp_ckmb_page_detection.py.")
        except Exception as e:
            print(f"Failed to initialize S3 client with IAM role: {str(e)} in torp_ckmb_page_detection.py.")
            if aws_access_key_id and aws_secret_access_key:
                self.s3_c = boto3.client('s3', 
                                    aws_access_key_id=aws_access_key_id,
                                    aws_secret_access_key=aws_secret_access_key)
                print("S3 client initialized successfully using manual keys in torp_ckmb_page_detection.py.")
            else:
                raise Exception("Unable to initialize S3 client. Check IAM role or provide AWS credentials in torp_ckmb_page_detection.py.")
    ...
    def get_page_meta_data_df(self, meta_data):
        ...
        data.columns = ['page', 'section', 'sub_section', 'table_element_match_list',
                        'panel_date', 'corpus_date', 'file_name']
        return data
    ...
    def table_checker(self, df):
        ...
        for idx, row in df.iterrows():
            if len(row['corpus_date']) >= 1 and len(row['table_element_match_list']) >= 1:
                tbl_ls.append(1)
            else:
                tbl_ls.append(0)
        df['is_table'] = tbl_ls
