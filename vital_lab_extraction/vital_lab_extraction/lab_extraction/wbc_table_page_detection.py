...
class WbcPanel:
    ...
    def get_corpus(self, d1):
        """ 
        function helps to get the corpus of given dataframe based on page number
        """
        corpus = ' '
    ...
    def get_page_meta_data(self):
        ...
        try:   
            ...
            print("S3 client initialized successfully using IAM role in wbc_table_page_detection.py.")
        except Exception as e:
            print(f"Failed to initialize S3 client with IAM role: {str(e)} in wbc_table_page_detection.py.")
            if aws_access_key_id and aws_secret_access_key:
                self.s3_c = boto3.client('s3', 
                                    aws_access_key_id=aws_access_key_id,
                                    aws_secret_access_key=aws_secret_access_key)
                print("S3 client initialized successfully using manual keys in wbc_table_page_detection.py.")
            else:
                raise Exception("Unable to initialize S3 client. Check IAM role or provide AWS credentials in wbc_table_page_detection.py.")
    ...
    def check_lab_str(self, lab_test):
        """ 
        The Scope of this Function is wbc Panel spacific (for table page detection)

        """
        cnt = 0
        lab_ls = ['wbc', 'white blood count', 'immature granulocyte', 'granulocyte', 'imm gran', 'grnn', 'abs imm gran',
                  'imm gran rel', 'immature granulocyte %', 'granulocytes', 'granulocyte %', '% immature granulocytes', 'immature gran', 'rbc', 'hgb', 'plt cnt', 'mono#', 'baso#']
    ...
    def table_checker(self, df):
            ...
            for idx, row in df.iterrows():
                if len(row['corpus_date']) >= 1 and len(row['table_element_match_list']) >= 3 and self.check_section(row['section'], row['zai_mapped_sub_section']) and self.check_lab_str(row['table_element_match_list']) and len(row['header_match_list']) >= 1\
                    or len(row['corpus_date']) >= 1 and len(row['table_element_match_list']) >= 2 and self.check_lab_str(row['table_element_match_list']) and len(row['header_match_list']) >= 1\
                        or len(row['corpus_date']) >= 4 and len(row['table_element_match_list']) >= 4 and self.check_lab_str(row['table_element_match_list']) and len(row['header_match_list']) >= 1\
                            or len(row['corpus_date']) >= 1 and len(row['table_element_match_list']) >= 2 and self.check_section(row['section'], row['zai_mapped_sub_section']):
                    tbl_ls.append(1)
                else:
                    tbl_ls.append(0)
            df['is_table'] = tbl_ls
