...
class ChemistryPanel:
    def __init__(self,bucket_name, csv_file, lab_test_list, lab_heading_list, master_section_list, master_subsection_list, exclude_regex_str=None):
        ...
        self.master_exclusion_section_list = master_subsection_list
    ...
    def get_panel_date(self, df):
        ...
        if len(idx_ls) > 0:
            for panel_idx in idx_ls:
                panel_idx_text = ' '.join(
                    df.loc[panel_idx-2:panel_idx+3, 'Text'].apply(lambda x:str(x)).to_list())

                panel_dates.append(
                    DatetimeExtractor.get_date_time_from_corpus_v2(panel_idx_text, ['none']))
                # detected_date = DatetimeExtractor.get_date_time_from_corpus_v2(panel_idx_text, [
                #                                                                'dob'])
                # if detected_date:
                #     panel_dates.append(detected_date[-1])

        # explode the list
        panel_dates = [item for sublist in panel_dates for item in sublist]
        # panel_dates = list(set(panel_dates))
        return panel_dates

    def get_page_meta_data(self):
        """ 
        args:
            self: ChemistryPanel Object
        return:
        ...
        try:   
            ...
            print("S3 client initialized successfully using IAM role in chemistry_table_page_detection.py.")
        except Exception as e:
            print(f"Failed to initialize S3 client with IAM role: {str(e)} in chemistry_table_page_detection.py.")
            if aws_access_key_id and aws_secret_access_key:
                self.s3_c = boto3.client('s3', 
                                    aws_access_key_id=aws_access_key_id,
                                    aws_secret_access_key=aws_secret_access_key)
                print("S3 client initialized successfully using manual keys in chemistry_table_page_detection.py.")
            else:
                raise Exception("Unable to initialize S3 client. Check IAM role or provide AWS credentials in chemistry_table_page_detection.py.")
        ...
        # df = pd.read_csv(self.csv_file, na_filter=False)
        ...
        for page in pg_list:
            ...
            meta_data.append((page, list(set(d1['section'])), list(set(d1['sub_section'])), table_element_match_list,
                              ...
                              panel_dates, corpus_dates, self.csv_file, list(set(d1['zai_mapped_sub_section']))))
        return meta_data
    ...
    def get_page_meta_data_df(self, meta_data):
        ...
        data.columns = ['page', 'section', 'sub_section', 'table_element_match_list',
                        'header_match_list', 'panel_date', 'corpus_date', 'file_name', "zai_mapped_sub_section"]
        return data
    ...
    # Table Page detection (Chemistry Spacific function)

    def check_chem_lab_str(self, lab_test):
        """ 
        The Scope of this Function is chemistry Panel spacific (for table page detection)

        """
        cnt = 0
        lab_ls = ['glucose', 'carbone dioxide', 'sodium', 'potassium', 'calcium', 'chloride', 'bun', 'creatinine', 'bilirubin', 'magnesium', 'phosphate',
                  'blood sugar', 'bs', 'co2', 'na', 'mg', 'po4', 'ca', 'phos', 'glu', 'pt', 'ptt', 'aptt', 'inr', 'protime', 'prothrombin time']
    ...
    def table_checker(self, df):
        ...
        for idx, row in df.iterrows():
            if (len(row['corpus_date']) >= 5 and len(row['table_element_match_list']) >= 5 and self.check_chem_lab_str(row['table_element_match_list']) and self.check_section(row['section'], row['zai_mapped_sub_section'])) or \
                len(row['table_element_match_list']) >= 2 and len(row['header_match_list']) >= 1 and self.check_chem_lab_str(row['table_element_match_list']) and self.check_section(row['section'], row['zai_mapped_sub_section']) or \
                    len(row['table_element_match_list']) >= 3 and len(row['header_match_list']) >= 1 and len(row['corpus_date']) >= 3 or\
                        len(row['table_element_match_list']) >= 2  and self.check_section(row['section'], row['zai_mapped_sub_section']):
                tbl_ls.append(1)
            else:
                tbl_ls.append(0)
        df['is_table'] = tbl_ls
