...
class BloodCulture:
    ...
    def get_page_meta_data(self):
        ...
        try:   
            ...
            print("S3 client initialized successfully using IAM role in blood_cult_page_detection.py.")
        except Exception as e:
            print(f"Failed to initialize S3 client with IAM role: {str(e)} in blood_cult_page_detection.py.")
            if aws_access_key_id and aws_secret_access_key:
                self.s3_c = boto3.client('s3', 
                                    aws_access_key_id=aws_access_key_id,
                                    aws_secret_access_key=aws_secret_access_key)
                print("S3 client initialized successfully using manual keys in blood_cult_page_detection.py.")
            else:
                raise Exception("Unable to initialize S3 client. Check IAM role or provide AWS credentials in blood_cult_page_detection.py.")
    ...
    def check_lab_str(self, lab_test):
        """ 
        The Scope of this Function is blood culture Panel spacific (for table page detection)

        """
        cnt = 0
        lab_ls = ['blood cult', 'blood culture', 'blood cx']
