...
try:
    from utility.get_bb import get_bb,vertical_alignment_detection
...
class VitalPanel:
    def __init__(self, bucket_name, csv_file, lab_test_list, master_section_list, master_subsection_list, master_exclusion_section_list=None):
    ...
    def get_page_meta_data(self):
        """ 
        args:
            self: VitalPanel object
        return:
            meta_data: list [(page_no, section, subsection,table_element_match_list,panel_dates,corpus_dates,file_name)]
        """
        try:   
            self.s3_c = boto3.client('s3', region_name='us-east-1')
            self.s3_c.list_buckets()
            print("S3 client initialized successfully using IAM role in vital_page_detection.py.")
        except Exception as e:
            print(f"Failed to initialize S3 client with IAM role: {str(e)} in vital_page_detection.py.")
            if aws_access_key_id and aws_secret_access_key:
                self.s3_c = boto3.client('s3', 
                                    aws_access_key_id=aws_access_key_id,
                                    aws_secret_access_key=aws_secret_access_key)
                print("S3 client initialized successfully using manual keys in vital_page_detection.py.")
            else:
                raise Exception("Unable to initialize S3 client. Check IAM role or provide AWS credentials in vital_page_detection.py.")
        ...
        for page in pg_list:
            ...
            # corpus_dates
            corpus_dates = DatetimeExtractor.get_date_time_from_corpus_v3(corpus, [
                'none'])
            
            #get bounding boxes
            prev_page_end_index = df[df['Page']<page]['End'].iloc[-1] if df[df['Page'] < page].shape[0] > 0 else 0
            dates_bb_ls = get_bb(d1, corpus_dates, prev_page_end_index)
            ...
            meta_data.append((page, list(set(d1['section'])), list(set(d1['sub_section'])), table_element_match_list,
                              ...
                              panel_dates, corpus_dates, dates_bb_ls, self.csv_file))
        return meta_data
    ...
    def get_page_meta_data_df(self, meta_data):
        ...
        data.columns = ['page', 'section', 'sub_section', 'table_element_match_list',
                        'panel_date', 'corpus_date', 'date_bounding_box', 'file_name']
        return data
    ...
    def table_checker(self, df):
        ...
        """
        tbl_ls = []
        previous_page_flag = None
        for idx, row in df.iterrows():
            if (len(row['corpus_date']) >= 1  and len(row['table_element_match_list']) >= 3 ) or (previous_page_flag and len(row['corpus_date']) >= 15 and vertical_alignment_detection(row['date_bounding_box'])) :
                df.at[idx, 'is_table'] = 1
                previous_page_flag = 1
            else:
                df.at[idx, 'is_table'] = 0
