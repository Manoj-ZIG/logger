...
class DatetimeExtractor:
    ...
    @staticmethod
    def get_date_tag(datetime_str, date_tags, date_start, prev_date_end, lookbehind_idx=18, ):
        """ 
        ...
        space_idx = [i.start()
                     for i in re.finditer(r"\s", datetime_str_original)]
    ...
    def get_date_time_from_corpus_v2(corpus, date_tags, restrict_dict={'day': (1, 31), 'month': (1, 12), 'year': (1900, 2026), 'year_': (1, 99)}):
        extracted_date = DatetimeExtractor.validate_date(corpus,range_dict= restrict_dict)
    ...
    def get_date_time_from_corpus_v3(corpus, date_tags, restrict_dict={'day': (1, 31), 'month': (1, 12), 'year': (1900, 2026), 'year_': (1, 99)}):
        extracted_date = DatetimeExtractor.validate_date(corpus,range_dict= restrict_dict)
        ...
        for detected_date in suppressed_date:
            ...
            if len(suppressed_time) > 0:
                time, _, time_start, time_end = suppressed_time[0]
                if time:
                    final_date_list.append((date, time, tag, (date_start, date_end)))
            else:
                final_date_list.append((date, None, tag, (date_start, date_end)))
            prev_date_end = date_end
        return final_date_list