...
def update_status(df,pdf_link, s3_path, status_code,file_name, bucket):
    ...
    df.loc[df['extrctn_pdf_links'] == pdf_link, 'dwnld_file_name'] = file_name
        ...
        for link in downloadable_links:
            try:
                ...
                if response.status_code == 200:
                    ...
                    s3_path = f'{DEST_FOLDER_LEVEL_1}/{DEST_FOLDER_LEVEL_2}/{PAYOR_NAME_DEST_FOLDER_LEVEL_3}/{DEST_FOLDER_LEVEL_4}/{DEST_FOLDER_LEVEL_5}/{DEST_FOLDER_LEVEL_6}/{DEST_FOLDER_LEVEL_8}/{file_name}'
                    s3_client.put_object(Bucket=bucket, Key=s3_path, Body=response.content, ContentType='application/pdf')
                    update_status(df, link,s3_path, response.status_code,file_name, bucket)
